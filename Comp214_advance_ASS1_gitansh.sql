...
--1.	List criminal(s) that Crime charges court fee is max.

select CRIMINALS.CRIMINAL_ID, CRIMINALS.FIRST, CRIMES.CRIMINAL_ID, CRIMES.CRIME_ID, CRIME_CHARGES.CHARGE_ID, CRIME_CHARGES.CRIME_ID, CRIME_CHARGES.COURT_FEE 
FROM CRIMINALS 
INNER JOIN 
CRIMES ON CRIMINALS.CRIMINAL_ID = CRIMES.CRIMINAL_ID 
INNER JOIN 
CRIME_CHARGES ON CRIMES.CRIME_ID = CRIME_CHARGES.CRIME_ID 
WHERE CRIME_CHARGES.COURT_FEE = (SELECT MAX(CRIME_CHARGES.COURT_FEE) FROM CRIME_CHARGES);

--2.    List criminals that have Min amount paid in crime charges.
select CRIMINALS.CRIMINAL_ID, CRIMINALS.FIRST, CRIMES.CRIMINAL_ID, CRIMES.CRIME_ID, CRIME_CHARGES.CHARGE_ID, CRIME_CHARGES.CRIME_ID, CRIME_CHARGES.AMOUNT_PAID 
FROM CRIMINALS 
INNER JOIN 
CRIMES ON CRIMINALS.CRIMINAL_ID = CRIMES.CRIMINAL_ID 
INNER JOIN 
CRIME_CHARGES ON CRIMES.CRIME_ID = CRIME_CHARGES.CRIME_ID 
WHERE CRIME_CHARGES.AMOUNT_PAID = (SELECT MIN(CRIME_CHARGES.AMOUNT_PAID) FROM CRIME_CHARGES);



--3.	List Officers that have max number of crimes assigned.
SELECT OFFICERS.OFFICER_ID, CRIME_OFFICERS.CRIME_ID
FROM OFFICERS
JOIN CRIME_OFFICERS ON OFFICERS.OFFICER_ID = CRIME_OFFICERS.OFFICER_ID
WHERE CRIME_OFFICERS.OFFICER_ID = (SELECT CRIME_OFFICERS.OFFICER_ID FROM  CRIME_OFFICERS GROUP BY CRIME_OFFICERS.OFFICER_ID HAVING COUNT(CRIME_OFFICERS.OFFICER_ID) = (SELECT MAX(COUNT(CRIME_OFFICERS.OFFICER_ID)) FROM CRIME_OFFICERS GROUP BY OFFICER_ID));

--4.	List criminals that have more than average sentences issued..
SELECT CRIMINALS.CRIMINAL_ID, CRIMINALS.FIRST, SENTENCES.SENTENCE_ID
FROM CRIMINALS 
JOIN SENTENCES ON CRIMINALS.CRIMINAL_ID = SENTENCES.CRIMINAL_ID
WHERE SENTENCES.CRIMINAL_ID = (SELECT SENTENCES.CRIMINAL_ID FROM SENTENCES GROUP BY SENTENCES.CRIMINAL_ID HAVING COUNT(SENTENCES.CRIMINAL_ID) > (SELECT AVG(COUNT(SENTENCES.CRIMINAL_ID))FROM SENTENCES GROUP BY CRIMINAL_ID));

--5.	List probation officers that have more than average criminals with sentences assigned with them.
SELECT PROB_OFFICERS.LAST, PROB_OFFICERS.PROB_ID 
FROM PROB_OFFICERS
WHERE PROB_ID = (SELECT PROB_ID FROM  SENTENCES GROUP BY PROB_ID HAVING COUNT(PROB_ID) >  (SELECT AVG(COUNT(PROB_ID))FROM SENTENCES GROUP BY PROB_ID));



--6.	List of criminals with less than average violations and having aliases.
SELECT CRIMINALS.CRIMINAL_ID, CRIMINALS.FIRST, ALIASES.ALIAS_ID, ALIASES.ALIAS
FROM CRIMINALS 
INNER JOIN SENTENCES ON CRIMINALS.CRIMINAL_ID = SENTENCES.CRIMINAL_ID
INNER JOIN ALIASES ON SENTENCES.CRIMINAL_ID = ALIASES.CRIMINAL_ID
WHERE SENTENCES.VIOLATIONS < (SELECT AVG(SENTENCES.VIOLATIONS) FROM SENTENCES);
